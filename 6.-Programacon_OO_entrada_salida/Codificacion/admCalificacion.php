				...
				<div class="card text-left">
					...
					<div class="card-body">
						<span class="btn btn-primary" data-toggle="modal" data-target="#agregarnuevosdatosmodal">
							Agregar nueva calificación <span class="fa fa-plus-circle"></span>
						</span>
						<hr>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="exampleModalLabel">Agregar nueva calificación</h5>
				...
				<div class="modal-body">
					<form id="frmnuevo">
						...
						<input type="text" class="form-control input-sm" id="id_asignatura" name="id_asignatura">
						<label>Calificación</label>
						<input type="text" class="form-control input-sm" id="calificacion" name="calificacion">
					</form>
				</div>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="frmnuevoU">
						...
						<label>id_asignatura</label>
						<input type="text" class="form-control input-sm" id="id_asignaturaU" name="id_asignaturaU">
	...
	$(document).ready(function(){
		$('#btnAgregarnuevo').click(function(){
			...
			$.ajax({
				type:"POST",
				data:datos,
				url:"procesos_calificacion/agregarCalificacion.php",
				success:function(r){
					if(r==1){
						$('#frmnuevo')[0].reset();
						$('#tablaDatatable').load('tablaCalificacion.php');
		...
		$('#btnActualizar').click(function(){
			...
			$.ajax({
				type:"POST",
				data:datos,
				url:"procesos_calificacion/actualizarCalificacion.php",
				success:function(r){
					if(r==1){
						$('#tablaDatatable').load('tablaCalificacion.php');
...
<script type="text/javascript">
	function agregaFrmActualizar(idalumno){
		$.ajax({
			...
			url:"procesos_calificacion/obtenDatosCalificacion.php",
			success:function(r){
				datos=jQuery.parseJSON(r);
				$('#idalumno').val(datos['id_alumno']);
				$('#id_asignaturaU').val(datos['id_asignatura']);
				$('#calificacionU').val(datos['calificacion']);
			}
		});
	}

	function eliminarCalificacion(idalumno){
		alertify.confirm('Eliminar calificacion', '¿Seguro desea eliminar calificacion?', function(){ 

			$.ajax({
				type:"POST",
				data:"idalumno=" + idalumno,
				url:"procesos_calificacion/eliminarCalificacion.php",
				success:function(r){
					if(r==1){
						$('#tablaDatatable').load('tablaCalificacion.php');
